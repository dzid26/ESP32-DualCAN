...
|---|---|---|
| `msg_sig_set(draft, "sig", val)` | `draft["data"] = signal_encode(draft["data"], sb, len, be, signed, scale, offset, val)` — full encode with scale/offset |
| `msg_sig_get(draft, "sig")` | `signal_decode(draft["data"], sb, len, be, signed, scale, offset)` — full decode with scale/offset |
