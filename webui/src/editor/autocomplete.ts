...
const API: BindingDoc[] = [
  ...
  { label: 'can_recv_raw',
    detail: 'can_recv_raw(bus:int [, msg_id:int]) -> list | nil | bytes | nil',
    documentation: 'Without msg_id: pop one received frame from the bus queue. Returns a list [id, data] — rx[0] = CAN ID, rx[1] = payload bytes. With msg_id: read latest rx data for that CAN ID directly as bytes.',
